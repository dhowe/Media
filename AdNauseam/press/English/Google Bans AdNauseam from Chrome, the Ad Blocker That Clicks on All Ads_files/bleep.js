...
{if(data['__board_offline__'])
{$('.bc_icons_wrap li a#bc_bell').removeClass('active');}}else{$('.bc_icon_box_bell>.bc_iconbox_content').html(data['html']);$('.bc_icons_wrap li a#bc_bell span').css('display','none');}}});}});});jQuery(document).on('copy',function(e)
{var sel=window.getSelection();$.ajax({type:"post",url:'/a/cplog.php',hideLoader:true,data:{url:document.location.href,t:sel.toString()},success:function(data){}});});