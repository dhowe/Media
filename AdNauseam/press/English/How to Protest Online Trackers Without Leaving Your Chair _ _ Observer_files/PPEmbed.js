...
var _cachedHost = 'http://pp-serve.newsinc.com'.slice(7);

var _ndnBuyerId = 'nyobserver_91843';
var distributorId = '91843';
var partnerId = '2291';
...
function recordUnavailablePerfectPixelPlacement(containerElementId) {
   // console.log('Recording unavailable Perfect Pixel placement for <div> with ID of "' + containerElementId + '".');

   _ndnq.push(['when', 'initialized', function() {
       _nw2e.require(['jquery', 'models/Ndn/Hooks'], function($, Ndn_Hooks) {
           var eventName = 'placementUnavailable',
               eventData = {
                   containerElement: $('#' + containerElementId).get(0)
               };
           Ndn_Hooks.getPublicApiAccess().trigger(containerElementId + '/' + eventName, eventData);
           Ndn_Hooks.getPublicApiAccess().trigger('*/' + eventName, eventData);
       });
   }]);
}
...
NDNRender.title = document.title.substring(0,150);

// To customize, scroll to "Start Customization"
...
NDNRender.getUnitDivIDs = function() {
    var ndnDivs = getElementsByClassName("ndn-widget");
    var ndnDivIDs = "";
    for (i = 0; i < ndnDivs.length; i++) {
        ndnDivIDs += ndnDivs[i].id + "|";
    }
    ndnDivs = getElementsByClassName("ndn_embed");
    for (i = 0; i < ndnDivs.length; i++) {
      if(ndnDivs[i].attributes['data-config-distributor-id']){
        ndnDivIDs += ndnDivs[i].id + "|";
      }
    }
    return ndnDivIDs;
...
(function($) {
  getPlacements = function(){
   // deffered.promise() allows an async function to prevent other code from interfering with the progress or state of its initial request   
  return $.Deferred(function(deferred){
    ...
    var placementsURL = ("https:" == document.location.protocol ? "https://" : "http://") + _ndnBkmHost
      ...
      + "&target_ids=" + NDNRender.getUnitDivIDs();
    ...
    // Will initiate a XDomainRequest in IE8 & IE9. IE10+ should skip to the ajax call below. We check first if XDomainRequest is there...then initiate a new crossDomainRequest
    if ('XDomainRequest' in window && window.XDomainRequest !== null && ie_version.major < 10) {				    
    ...
    else{ // All other browsers except IE versions < 11
  ...
  }).promise(); // returns a deferred's promise object
}
...
(function($){
 ...
 getPlacements().done(function(placements){				
	window.placements = placements.placements;
    ...
    _nw2e.jQuery('.ndn_embed:not(.ndn_embedded,[data-config-legacy-embed-url])[data-config-distributor-id="' + distributorId + '"]').each(function() {
      ...
      if(_legacyEmbedUrl){
       ...
       var floatBehavior = getUrlParameters("float", _legacyEmbedUrl, true);
       ...
       recordUnavailablePerfectPixelPlacement(_nw2e.jQuery(this).attr('id'));
      }
