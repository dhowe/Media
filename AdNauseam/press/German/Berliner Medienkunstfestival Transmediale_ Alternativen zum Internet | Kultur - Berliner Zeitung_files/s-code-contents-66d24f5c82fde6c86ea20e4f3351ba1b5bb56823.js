var s_account=(typeof s_config != "undefined") ? s_config.account_id : ( ( (typeof dataLayer != "undefined") && (typeof dataLayer.account_id != "undefined" )) ? dataLayer.account_id: "unknown" );
var s=s_gi(s_account);


//s = new AppMeasurement();
/*
s.account = s_config.account_id;
s.linkInternalFilters = s_config.linkInternalFilters;
*/

/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="dumontnet"
s.trackingServer="dumontnet.d3.sc.omtrdc.net"
...
s.linkInternalFilters="javascript:,"+ window.location.host + (s_config.linkInternalFilters?","+s_config.linkInternalFilters:"");
//10.10.2014 lg 
if (window.location.href.indexOf("ksta.de") > -1) {
  s.linkInternalFilters += ",piano.ksta.de,bar.ksta.de";
}
//3.2.2015 lg 
if (window.location.href.indexOf("berliner-zeitung.de") > -1) {
  s.linkInternalFilters += ",piano.berliner-zeitung.de,bar.berliner-zeitung.de";
}
//12.5.2015 rz for stage testing 
if (window.location.href.indexOf("rundschau-online.de") > -1) {
  s.linkInternalFilters += ",mobil.stage.rundschau-online.de,web.stage.rundschau-online.de,www.rundschau-online.de";
}


s.linkLeaveQueryString=true; // changed to true 31.10.12
s.linkTrackVars="prop12"; // for Exit Link Tracking 31.10.12
s.linkTrackEvents="None";

//                2012         2013         2014         2015         2016         2017    
s.dstStart = ["03/25/2012","03/31/2013","03/30/2014","03/29/2015","03/27/2016","03/26/2017"][(new Date).getFullYear()-2012];  // update to the correct Daylight Savings Time start date for the current year.
s.dstEnd   = ["10/28/2012","10/27/2013","10/26/2014","10/25/2015","10/30/2016","10/29/2017"][(new Date).getFullYear()-2012];  // update to the correct Daylight Savings Time end date for the current year.   
s.currentYear = (new Date).getFullYear(); // update to the current year (can be done programmatically).   

/* Plugin Config */
s.usePlugins=true;

function s_doPlugins(s_s) {
	/* Add calls to plugins here */
	
  //only in case events are set
  if (typeof  s.events != "undefined") {
  
    /* Valiton Config 16.07.2012 */	
    /* Identify external Campaigns from URL Query String    */
    /* Do not set s.campaign in pic galleries ++ added condition 9.7.2013*/
  
    if (s.events.indexOf("event25") == -1) {		
        s.campaign = s.Util.getQueryParam("dmcid");
    } else {
        s.campaign = "";
    }	
  
    // Identify internal Campaigns/Teaserboxes from URL Query String 
    // Do not set eVar42 in pic galleries ++ added condition 8.7.2013
    
    if (s.events.indexOf("event25") == -1) {		
          s.eVar42 = s.Util.getQueryParam('iid');
    } else {
        s.eVar42 = "";
    }
	...
	// Channel
//	s.channel = "D=v3"; //set in the rule
	
	// Hierarchy
//	s.hier1 = "D=v3"; //16.04. DTM

	// Ressort_Subressort (2nd content level) from eVar3 (Rubrik) 
  // uebertragen in pageload rule, LG, 23.04
//	s.eVar4 = s.eVar3.split("_").slice(0, 2).join("_");
	
	// Copy pageName
	//s.eVar10 = s.prop12 = "D=pageName";
	
	// URL
	s.eVar11 = "D=g";

	//date from unix date(e.Var19)
	s.eVar12 = valiton.dateString(s.eVar19);
	
	// Track Kanal only once per visit
  // removed after increasing amount of Nones, LG, 20.05.14
  //if(s.prop6){s.eVar14=s.getValOnce(s.prop6, 's_v14', 0);} 
	
	// Track RefDom for Classification
//if (window.location.href.indexOf("ksta.de") == -1) {
//	s.eVar15 = valiton.referringDomain();
//}
	
	// Time Parting
	s.eVar18 = s.getTimeParting('w', '+1') + "|" + s.getTimeParting('d', '+1') + "|" + s.getTimeParting('h', '+1');

	//calculate minutes since article release
	if (s.eVar17 != "" && s.eVar19 != "") {
		s.eVar30 = s.eVar19 - s.eVar17;
	};
	
	//Copy Referrer to eVar	
	s.eVar34 = "D=r";

	//Track SEO Keyword
	s.eVar35 = valiton.searchenginekeyword(window.document.referrer);
	
	/* Previous Page */
  s.eVar41 = s.getPreviousValue(s.pageName,'gpv_pn','event1');
  	
	// Previous Sub Ressort
  s.eVar44 = s.getPreviousValue(s.eVar4,'gpv_r','event1'); //14.04. DTM

	/* Flash version detection -- DEPRECATED 06.03.2014*/
	//s.detectRIA('s_ria','eVar47','','','','1');

	/* Piano UID */
/*	if (typeof(Storage)!=="undefined" && typeof localStorage.valPianoUID !== 'undefined'){
		s.eVar50 = localStorage.valPianoUID;
	} else {
		s.eVar50 = "";
	} */
	...
	// Copy eVars to props
	/*s.prop1="D=v1";  // Site
	s.prop2="D=v2";  // Ressort
	s.prop3="D=v3";  // Kategorie
	s.prop4="D=v8";  // Adzone
	s.prop5="D=v11";  // URL
	s.prop6="D=v14";  // Kanal
	s.prop7="D=v15";  // Referrer
	s.prop8="D=v17";  // Datetime
	s.prop9="D=v18";  // Time Parting
	s.prop10="D=v19";  // Datum
	s.prop13="D=v41"; //Previous Page
	s.eVar13="D=v12"; //copy date to eVar13 (last) 
	s.eVar43="D=v28"; // Anzahl Artikel gelesen (added 18.01.13)
	s.prop15="D=v45"; // mobile Ansicht (added 18.06.13)
	s.prop16="D=v9"; // Seitentyp in prop speichern (added 9.10.13)
	s.prop17="D=v50"; // Piano UID
	s.prop18="D=v7"; // Artikelueberschrift
	*/
	/* End Valiton Config */

	// clear plugins variable (depr. V15) shorten image request
	s.plugins = "";
}
s.doPlugins = s_doPlugins;

/******************** Valiton Custom Functions **********************/
var valiton = {};


// 31.10.12 added Usage of internalFilters
valiton.referringDomain = function (){
	var referrer = "";

	if ((typeof _satellite.getVar('p_referingDomain') != "undefined") && (_satellite.getVar('p_referingDomain') != "undefined") && (_satellite.getVar('p_referingDomain') != "unknown") && (_satellite.getVar('p_referingDomain') != "")) {
		// Piano first-time user whitescreen
		referrer =  _satellite.getVar('p_referingDomain');
    console.log ('REFERRER: ' + _satellite.getVar('p_referingDomain'));
		_satellite.removeCookie('piano_referer_bar2');
	} else {
	...
	if ((referrer != "") && (referrer != "-")) {
        ...
        // interne Referrer filtern (Funktion wie siehe unten)      
        var domains = s.linkInternalFilters.split(',');
		        for(var i = 0, l = domains.length; i < l; i++ ){
		            if(res[1].indexOf(domains[i]) > -1){ 
                var detect_internal_referrer = "1";
                }
              }      
              
        // START: Special Social Media Tracking Codes
        
        // Referrer Facebook AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_fb_p/)) && (detect_internal_referrer != 1) && (res[1].match(/facebook/))) {        
          referrer =  res[1] + '::Facebook Page';
          console.log ('REFERRER: ' + res[1] + '::Facebook Page');    
        } else {
        // Referrer Facebook AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_fb/)) && (detect_internal_referrer != 1) && (res[1].match(/facebook/))) {
          referrer =  res[1] + '::Facebook Share';
          console.log ('REFERRER: ' + res[1] + '::Facebook Share');  
        } else {
        // Referrer Twitter AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_tw_p/)) && (detect_internal_referrer != 1) && (res[1].match(/twitter|t.co/))) {
          referrer =  res[1] + '::Twitter Page';
          console.log ('REFERRER: ' + res[1] + '::Twitter Page');
        } else {
        // Referrer Twitter AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_tw/)) && (detect_internal_referrer != 1) && (res[1].match(/twitter|t.co/))) {
          referrer =  res[1] + '::Twitter Share';
          console.log ('REFERRER: ' + res[1] + '::Twitter Share');
        } else {
        // Referrer Instanews AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_wn/)) && (detect_internal_referrer != 1) && (res[1].match(/instanews/))) {
          referrer =  res[1] + '::WhatsApp News';
          console.log ('REFERRER: ' + res[1] + '::WhatsApp News');
        } else {
        // Referrer that does NOT match twitter, facebook AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_wn/)) && (detect_internal_referrer != 1) && (!res[1].match(/twitter|t.co|facebook/))) {
          referrer =  res[1] + '::WhatsApp News';
          console.log ('REFERRER: ' + res[1] + '::WhatsApp News');
        } else {
        // Tracking Code WhatsApp
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_wa/)) && (detect_internal_referrer != 1)) {
          referrer =  res[1] + '::WhatsApp Share';
          console.log ('REFERRER: ' + res[1] + '::WhatsApp Share');
        } else {
        // Referrer Google AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_gp/)) && (detect_internal_referrer != 1) && (res[1].match(/google/))) {
          referrer =  res[1] + '::GooglePlus Share';
          console.log ('REFERRER: ' + res[1] + '::GooglePlus Share');
        } else {
        // Referrer does NOT match Facebook, Twitter, Google AND Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^sm_em/)) && (detect_internal_referrer != 1) && (!res[1].match(/twitter|t.co|facebook|google/))) {
          referrer =  res[1] + '::Mail Share';
          console.log ('REFERRER: ' + res[1] + '::Mail Share');
        } else {
        // Tracking Code
        if ((s.campaign != undefined) && (s.campaign.match(/^f_msn_web/)) && (detect_internal_referrer != 1)) {
          referrer =  res[1] + '::MSN Feed';
          console.log ('REFERRER: ' + res[1] + '::MSN Feed');
        } else {
        
        // END: Special Social Media Tracking Codes 
        
				referrer =  res[1];
        console.log ('REFERRER (Standard): ' + res[1]);
			    var doms = s.linkInternalFilters.split(',');
		        for(var i = 0, l = doms.length; i < l; i++ ){
		            if(res[1].indexOf(doms[i]) > -1){                
						 if ((_satellite.getVar('mobileSwitchReferrer') !="") && (_satellite.getVar('mobileSwitchReferrer') !="no_mobileswitch_referrer")) {
						 referrer = _satellite.getVar('mobileSwitchReferrer');
             console.log ('REFERRER: ' + _satellite.getVar('mobileSwitchReferrer'));
						 } else {
							// internal referrer
									referrer = "";
                  console.log ('REFERRER: internal Referrer');
						}
		            }
		        }	
            }
            
            //Closing brackets of if-cases
            }
            }
            }
            }
				...
				referrer = "Unknown - invalid referrer";
        console.log ('REFERRER: Unknown - invalid referrer');
			}
		} else {
			referrer = "Unknown - invalid browser";
      console.log ('REFERRER: Unknown - invalid browser');
		}
	} else { 
    
    if ( _satellite.getVar('appUserAgent')!="") {
      // no referrer, but special user agent is set (FB app, DuMont app)
      referrer = _satellite.getVar('appUserAgent');
      console.log ('REFERRER: AppUserAgent Referrer');
    } else {
      // no referrer BUT tracking code (includes the former "Typed/Bookmarked" instances)
      if ((s.campaign != undefined) && s.campaign.match(/^sm_fb|^sm_fb_p|^sm_wn|^sm_wa|^sm_tw|^sm_tw_p|^sm_gp|^f_msn_web|^sm_em/)) { 
        
        if (s.campaign.match(/^sm_fb_p/)) {
          referrer = "Facebook Page";
          console.log ('REFERRER: Facebook Page');
        } else {
          if (s.campaign.match(/^sm_fb/)) {
            referrer = "Facebook Share";
            console.log ('REFERRER: Facebook Share');
          } else {
            if (s.campaign.match(/^sm_tw_p/)) {
              referrer = "Twitter Page";
              console.log ('REFERRER: Twitter Page');
            } else {
              if (s.campaign.match(/^sm_tw/)) {
                referrer = "Twitter Share";
                console.log ('REFERRER: Twitter Share');
              } else {
                if (s.campaign.match(/^sm_wn/)) {
                  referrer = "WhatsApp News";
                  console.log ('REFERRER: WhatsApp News');
                } else {
                  if (s.campaign.match(/^sm_wa/)) {
                    referrer = "WhatsApp Share";
                    console.log ('REFERRER: WhatsApp Share');
                  } else {
                   if (s.campaign.match(/^sm_gp/)) {
                      referrer = "GooglePlus Share";
                      console.log ('REFERRER: GooglePlus Share');
                   } else {
                    if (s.campaign.match(/^sm_em/)) {
                      referrer = "Mail Share";
                      console.log ('REFERRER: Mail Share');
                    } else {
                      if (s.campaign.match(/^f_msn_web/)) {
                       referrer = "MSN Feed";
                       console.log ('REFERRER: MSN Feed');
                      }
                     }
                    }
                   }
          ...
          console.log ('REFERRER: Typed/Bookmarked'); 
          } 
        }
	}
...
valiton.searchenginekeyword = function (refurl) {
    if ((refurl != "") && (refurl != "-")) {
        var qm = refurl.indexOf('?');
        if (qm > -1) {
            dom = refurl.substr(0, qm);
            qstr = refurl.substring(qm, refurl.length);
              } else {
            dom = refurl;
            qstr = "";
        }
        ...
        SE[10] = ['abacho.', 'q'];
        SE[11] = ['alltheweb.', 'q'];
        SE[12] = ['search.lycos.', 'query'];
        SE[13] = ['.dino-online.', 'query'];
        SE[14] = ['.gmx.', 'su'];
        SE[15] = ['freenet.', 'query'];
        SE[16] = ['bing.', 'q'];
        ...
        for (var i = 0; i < SE.length; i++) {
            if (dom.indexOf(SE[i][0]) > -1 && dom.indexOf("news.google") == -1){                
                if (qstr.indexOf(SE[i][1]) > -1) {
					SEOquery = s.Util.getQueryParam(SE[i][1],'',refurl);
					if (SEOquery != ""){
						SEOkw = SEOquery;
					} else {
						SEOkw = "Keyword unavailable";
					}	
                } else {
                    SEOkw = "Keyword unavailable";
                }
...
valiton.dateString = function (unixDate) {

var valDate = new Date(unixDate*60000); //converts minutes to milliseconds
var valYear = valDate.getFullYear();
var valMonth = valDate.getMonth()+1;
var valDay = valDate.getDate();
var formatDate = valYear + '-' + valMonth + '-'+ valDay;
