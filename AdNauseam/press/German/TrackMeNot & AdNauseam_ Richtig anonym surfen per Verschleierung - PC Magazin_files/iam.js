  ...
  function loadSurvey() {
    ...
    var getInvitation = function (response) {
      if (response && !response.hasOwnProperty("errors")){
        var isEligibleForInvitation = ( "NONE" === response.data.identityStatus.blockStatus.toUpperCase() );
        if (isEligibleForInvitation) {
          if (IAMQSElement) {
            IAMQSElement.parentNode.removeChild(IAMQSElement);
          }
          IAMQSElement = createScriptTag(response.links.surveyinvitation);
        }
      }
    };
    ...
    if (sv && !surveyCalled && result.sv !== "ke" && (result.sv === "in" || result.sv === "mo" || result.sv === "i2" )) {
      ...
      if (result.sv === "in") {
        document.write("<script src='"+window.location.protocol+'//qs.ioam.de/?'+((window.szmvars)?szmvars:"")+"'></script>");
      } else if (result.sv === "i2") {
        if (IAMQSElement) {
          IAMQSElement.parentNode.removeChild(IAMQSElement);
        }
        IAMQSElement = createScriptTag(window.location.protocol+'//qs.ioam.de/?'+((window.szmvars)?szmvars:""));
      } else if (result.sv === "mo") {
        ...
        var checkForInvitationUrl = protocol + '//' + qdsParameter.host + ':' +  qdsParameter.port + '/' + pathOfCheckInvitation + queryParameterString;
