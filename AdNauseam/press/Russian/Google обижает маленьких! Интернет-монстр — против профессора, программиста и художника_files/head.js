...
try {
  ...
  var widgetsCfg = {
   "rFOsi0uLx0-1I5mZ" : {
      "payload" : {
         "insertBeforeElemSelect" : ".relapWidget",
         ...
         "testVariants" : {
            "1" : {
               ...
               "cfg" : {
                  ...
                  "descriptionFont" : "",
                  "preserve_rows_quantity" : "",
                  "titleFontSize" : "",
                  "theme" : "",
                  "titleMarginTop" : "",
                  "headLinesCount" : "",
                  "adLabel" : "Cпонсорский контент",
                  "cols" : "",
                  "with_block_title" : "",
                  "windowMinWidth" : "",
                  "illustrationType" : "",
                  "cutTextUntilThumbnailBottom" : "",
                  "illustrationSize" : "",
                  "descriptionFontSize" : "",
         ...
         "waitForElemTimeout" : 1,
         "theme" : "novayagazeta_ru__platerespons",
         "cols" : 4,
         "adLabel" : "",
         "headLinesCount" : 3,
         "with_footer" : 0,
      ...
      "company_id" : "RT1FYORgJFumG46L",
      "id" : "rFOsi0uLx0-1I5mZ",
      "user_id" : "RT1FYORgJFumG46L",
      "domain_id" : "CGjeOg",
      "is_enabled" : true
   }
}
;
  ...
  var extConfig = {
   "CNAAC" : {
      "src" : "https://relap.io/cc?_s=UEG2wQ",
      "type" : "jsonp",
      "timeout" : 800,
   ...
   "xV5cwg" : {
      ...
      "timeout" : 2500,
      "callbackParamName" : "callback",
      "src" : "https://ad.mail.ru/adq/?q=45459&cpm_floor=1000&sitename=novayagazeta.ru&count=1"
   },
   "63rReg" : {
      "type" : "img",
      "timeout" : 500,
      "src" : "https://recreativ.ru/mtch/21/Wiu3g3a6?r=https%3A%2F%2Frelap.io%2Fapi%2Fpartners%2Frccs.gif%3Fuid%3D%24UID%26ruid%3D%24SID"
   },
   "hSBNIQ" : {
   ...
   "pCYNWw" : {
      "src" : "https://cookiefwd.buzzoola.com/forward?redirect_url=https%3A%2F%2Frelap.io%2Fapi%2Fpartners%2Fbzcs.gif%3F_s%3DUEG2wQ%26c%3D%7Bbuzz_cookie%7D",
      "type" : "img",
      "timeout" : 1000
   },
   "gIujSQ" : {
      "src" : "https://sync-eu.exe.bid/image?source=relap&return_url=%2F%2Frelap.io%2Fapi%2Fpartners%2Fdccs.gif%3Fuid%3D%7BUID%7D&id=Wiu3g3a6",
      "type" : "img",
      "timeout" : 500
   }
}
;
  ...
  var targetUrl = 'https://www.novayagazeta.ru/articles/2017/01/19/71209-google-obizhaet-malenkih';
  getTargetUrl();

  debugLog(targetUrl, "url");

  var seed = 'FnSPcFoB0GjC3yTfdKI';
...
function ExtConfigurer(opt) {
  ...
  function loadVast_2_0(cfgId) {
    ...
    function gpmdError() {};
    if (urlParts['params'] && urlParts.params['puid2'] && urlParts.params.puid2 == 511) {
      self._gpmdError = '';
      function gpmdError(error) {
        self._gpmdError = error;
      }
    }

    if (!w.sessionStorage) {
      log("browser doesn't support sessionStorage: " + cfgId);
      gpmdError('nosessionstorage');
      cancelRequest('manual');
      return;
    }

    var timeoutId = setTimeout(cancelRequest, totalTimeout);
    ...
    function cancelRequest(manual) {
      ...
      if (!manual) gpmdError('globaltimeout');
      if (!manual && request) {
    ...
    function onAjaxComplete(event) {
      ...
      if (!event || !event.type || event.type === 'error' || event.type === 'timeout') {
        ...
        gpmdError('ajaxerror.' + (event.type || 'unknown'));
        cancelRequest('manual');
          ...
          if (link.hasAttribute('data-inline-video')) {
            ...
            if (inlineVideoIframe) {
              ...
              function loadVastConfig(vastLoadConfig) {
                ...
                (new Image()).src = 'https://relap.io/va/cmpl/CAMPAIGN_OBFUSCATED_ID/00?r='.replace('CAMPAIGN_OBFUSCATED_ID', 'fQ25BA') + link.getAttribute('data');
                var extParams = [];
                var extConfigurer = new w.relap.ExtConfigurer({
                  cfg: vastLoadConfig,
                  extParams: extParams,
                  complete: function() {
                    gpmdGraphite('loadconfigcomplete');
                    if (extConfigurer._gpmdError) gpmdGraphite('errs.' + extConfigurer._gpmdError);
                    if (!extParams[0]) return; /* vast не загрузился */
                    gpmdGraphite('loadconfigsuccess');
                    /* достаём из expParams путь, по которому сохранён полный vast */
                    sendVastToIframe(extParams[0].split('=', 2)[0]);
                    gpmdGraphite(
                      rectsIntersect(getElementRect(el), getPageRect()) ?
                      'vastsent_still_intersect' :
                      'vastsent_not_intersect'
                    );
                  }
                });
              }
...
var gpmdPlaySent = false;
              function checkIframeIsVisibleEnough() {
                var intersection = rectsIntersectPercentage(getElementRect(inlineVideoIframe), getPageRect());
                ...
                if (intersection >= 0.25) {
                  ...
                  if (!gpmdPlaySent && inlineVideoIframe.src.match(/^https:\/\/relap.io\/va\/i\/fQ25BA/)) {
                    gpmdPlaySent = true;
                    gpmdGraphite('play');
                  }
                } else {
              ...
              function gpmdGraphite(counter) {
                (new Image()).src = 'https://relap.io/d/c/g?counter=count.ads.dbg.2603.' + counter;
              }
  ...
  function checkBlockScrolledTo() {
    ...
    try {
      ...
      if (!rectsIntersect(getPageRect(), getElementRect(topContainerEl))) return;
      ...
      for (var i = 0; i < linksSize; i++) {
        ...
        if (onShowAttr || isAd) {
          ...
          else if (rectsIntersect(getPageRect(), getElementRect(link))) {
